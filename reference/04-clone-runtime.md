...
- "Update the clone" → run the **append-only delta cycle**, not a full rebuild: `context` (the `ep` table's entities + vector map as pressure) → extract the new source's EPs (grounded, routed) → `smoke <delta>` (dry-run until clean) → `import <delta>` (append-only into the `ep` table). The table only grows; nothing is overwritten. See `reference/07-ep-store.md`.
